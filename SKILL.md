...
An effort too big for one session fails a specific way: the outcome drifts from the intent, one unshared assumption at a time. grill-master prevents that drift by designing the workflow as it runs: a loose idea becomes a **working brief** — the operator's intent made checkable — and sessions work it one step at a time until the finish line holds.

**The workflow unfolds.** A normal workflow is drawn before it runs; this one is discovered. The unfolding rule governs every session: take one step, then re-read the brief as a whole and record what the step changed. Each step is structure-preserving (Alexander) — there is no territory to discover, only a whole to strengthen, one step at a time.
...
In everything the human reads, call briefs and steps by their titles with the file linked — a wall of slugs is illegible.

## The brief

One directory per effort: `.brain/projects/<effort>/` — the brief file, one file per step, any assets. The brief is an **index**, not a store: each resolution lives in its step file. Its sections:
...
- **Vocabulary** — the terms this effort has settled, one line each: the effort's pattern language. A record of negotiations, not a glossary — words don't carry meaning by themselves, and drift is exactly that assumption failing — so each line is a settlement between operator and agent. A term used two ways silently forks the work; any session that catches drift re-negotiates the term on the spot and lands the settled meaning before closing.
...
- **Decision ledger** — one line per closed step: name-link plus gist. Closed is not solved forever: when reality reopens a settled decision, charter a new step naming what it supersedes — the old resolution never gets edited, and the reappearance is natural, not failure.
- **Latent** — unknowns already in the whole but too soft to phrase as steps: the effort's latent structure. The test: can you state the step precisely now, even if you can't take it yet? Sharp → its own step file (blocked is fine). Soft → one line here, graduating when a result develops it.
...
Open steps are **not** listed in the brief; the scan finds what's ready (defined in CONVENTIONS.md).
...
1. **Pin the finish line** with `grill-me`. Look up facts yourself; put only decisions to the human. Terms settle as the finish line does — seed the Vocabulary with them. Done when the human confirms finish-line conditions they could test an outcome against.
2. **Fan out breadth-first** — grill across the whole space, not deep on any thread, surfacing decisions, build phases, and soft unknowns. Charter the earliest touchable prototype the space allows — a reaction beats a discussion. If everything fits in this one session, skip the brief and say so. Done when the human has nothing more to add at this altitude.
