...
The tracker is the filesystem: `.brain/projects/<effort>/` in the repo that owns the effort (whichever repo owns the effort). Files are `.svx` in my MDSvX-based setup; plain `.md` works identically if you don't run one — adjust the extension throughout. Frontmatter is the machine surface; the scan below reads it, so follow it exactly.
